(() => {
  ...
  function addDashboardLink(){const actions=$('.header-actions');if(!actions||$('.dashboard-link'))return;const link=document.createElement('a');link.className='header-button dashboard-link';link.href='pages/dashboard.html';link.dataset.i18n='dashboard_nav';link.dataset.i18nAriaLabel='dashboard_nav';link.setAttribute('aria-label',t('dashboard_nav','Tableau de bord'));link.title=t('dashboard_nav','Tableau de bord');link.textContent=t('dashboard_nav','Tableau de bord');actions.prepend(link);}
  function addReportLink(){const actions=$('.header-actions');if(!actions||$('.report-link'))return;const link=document.createElement('a');link.className='header-button report-link';link.href='pages/rapport.html';link.dataset.i18n='report_nav';link.dataset.i18nAriaLabel='report_nav';link.setAttribute('aria-label',t('report_nav','Rapport'));link.title=t('report_nav','Rapport');link.textContent=t('report_nav','Rapport');actions.prepend(link);}
  ...
  function bind(){addProjectsLink();addReportLink();addDashboardLink();prepareNotification();addNbprofLink();
